...
let
    buildHaskell = {year, day, bin ? []}:
        let ghcWithPackages = pkgs.haskell.packages."${ghc}".ghcWithPackages
                (p: [
                    p.hashable
    ...
    buildClojure = {year, day, suffix ? ""}:
        ...
        in  pkgs.stdenv.mkDerivation rec {
            ...
            buildPhase = ''
                ...
                echo 'exec ${clojure} -Scp "$CLJCP" -M ${./${year}/${day}.clj}' >>${name}
                chmod +x ${name}
            '';
            installPhase = "mkdir $out; cp ${name} $out";
        };

    buildC = {year, day, suffix ? ""}: pkgs.stdenv.mkDerivation rec {
        name = "${year}-${day}${suffix}";
        src = ./${year};
    ...
    buildCpp = {year, day, suffix ? ""}: pkgs.stdenv.mkDerivation rec {
        name = "${year}-${day}${suffix}";
        src = ./${year};
    ...
    buildGo = {year, day, suffix ? ""}: pkgs.stdenv.mkDerivation rec {
        name = "${year}-${day}${suffix}";
        src = ./${year};
        buildPhase = ''
    ...
    buildOCaml = {year, day, suffix ? ""}: pkgs.stdenv.mkDerivation rec {
        name = "${year}-${day}${suffix}";
        src = ./${year};
        buildPhase = ''
...
in  rec {
    all = pkgs.symlinkJoin {
        name = "advent-of-code";
        paths = builtins.concatMap (pkgs.lib.attrsets.mapAttrsToList (k: v: v))
            [y2015 y2017 y2018 y2019 y2020 y2021 y2022 y2023];
    };
     y2015 = {
    ...
    y2021 = {
        ...
        d24 = buildPython  {year = "2021"; day = "24"; packages = (p: [p.z3]);};
